...
\chapter{Detailed \glsfmtshort{AI} Taxonomy Descriptions}
\label{app:taxonomy}

This appendix provides a detailed breakdown of the \\AIDpackageName\ taxonomy.
For each category, we define the \textbf{Objective} (the high-level goal of using \gls{AI} in this phase) and the \textbf{Scope} (what is generally included or excluded).
...
\paragraph{Objective:} To use \gls{AI} as a thought partner for brainstorming, refining the research direction, and establishing the theoretical foundation before empirical work begins.
...
\paragraph{Keys:}
\begin{description}
    \item[Idea generation:] Using \gls{AI} to brainstorm new research topics, interdisciplinary connections, or novel angles on existing problems.
    ...
    \item[Simulation/Scenarios:] Using \gls{AI} to conceptualize theoretical models, simulate persona responses, or design hypothetical scenarios.
\end{description}
...
\paragraph{Keys:}
\begin{description}
    \item[Search \& Discovery:] Using \gls{AI} tools (e.g., semantic search) to find relevant papers that keyword searches might miss.
    ...
    \item[Gap identification:] Using \gls{AI} to suggest areas where current research is lacking or contradictory.
...
\paragraph{Keys:}
\begin{description}
    \item[Collection:] Writing scrapers or using \gls{AI} agents to gather public data.
...
\paragraph{Objective:} To use \gls{AI} as a critical reviewer or “devil's advocate.”
\paragraph{Scope:} Includes simulated peer review and limitation checking.
\paragraph{Keys:}
\begin{description}
    \item[Simulated Peer Review:] Using \gls{AI} as a reviewer to identify unclear sections or potential critiques.
