...
%% ----------------------------------------------------------------------------
%%  novathesis — 3-BackMatter/app-covers.tex
%% ----------------------------------------------------------------------------
%%
...
\chapter{The \texorpdfstring{\textbf{nova}thesis}{novathesis} Cover Page System}
\label{app:covers}
...
\begin{enumerate}
  \item \textbf{\ntindex[covers!]{Background colour}.}  A solid \latexinline{\pagecolor{}} fills the
        entire page.  Set with:
        \begin{latexcode}
\thesiscover(id,bgcolor):={colorname}
        \end{latexcode}
  \item \textbf{\ntindex[covers!]{Background image}.}  A full-bleed graphic is stretched to
...
\medskip
\noindent\textbf{Element-level parameters (\latexinline{\ntaddtocover[options]{covers}{content}}):}

\begin{center}
\small
\begin{ShadedTabular}[white]{@{}llll@{}}
\hline
\textbf{Attribute} & \textbf{Values} & \textbf{Default} & \textbf{Description} \\
\hline
\texttt{status} & \makecell[lt]{\texttt{working},\\\quad\texttt{provisional},\\\quad\texttt{final}} & (all) & Filter by doc status \\
\texttt{degree} & \texttt{phd}, \texttt{msc}, \texttt{bsc}, \ldots & (all) & Filter by degree type \\
\hline
\texttt{width}  & \textit{dim} & text width & Box width \\
\texttt{height} & \textit{dim} & natural & Fixed box height \\
\makecell[lt]{\texttt{height/phd},\\\quad\texttt{height/msc}} & \textit{dim} & — & Per-doctype height override \\
\texttt{minheight} & \textit{dim} & \texttt{0pt} & Minimum box height \\
\texttt{hspace} & \textit{dim} & \texttt{0pt} & Horizontal offset from left margin \\
\hline
\texttt{halign} & \texttt{l}, \texttt{c}, \texttt{r}, \texttt{j} & \texttt{c} & Horizontal content alignment \\
\texttt{valign} & \texttt{t}, \texttt{c}, \texttt{b} & \texttt{c} & Vertical content alignment (needs \texttt{height}) \\
\hline
\texttt{color}  & \textit{colorname} & (page default) & Foreground text colour \\
\texttt{bgcolor} & \textit{colorname} & transparent & Box background colour \\
\hline
\texttt{vspace} & \textit{dim} & \texttt{0pt} & Fixed vertical gap \emph{before} the element \\
\texttt{vfill}  & \textit{integer} & — & Elastic gap before element (\textit{n}×\texttt{fill}) \\
\hline
\texttt{tikz}   & \texttt{true}, \texttt{false} & \texttt{false} & Treat content as raw TikZ overlay code \\
\texttt{newpar} & \texttt{true}, \texttt{false} & \texttt{true} & Emit \latexinline{\par} after element \\
\hline
\end{ShadedTabular}
\end{center}
...
Two attributes control the vertical gap \emph{before} the element.  Use
\texttt{vspace} for an absolute dimension, or \texttt{vfill} for elastic
(stretchable) spacing:

\begin{description}
  \item[\texttt{vspace=\{<dim>\}}] Insert a fixed \latexinline{\vspace*{<dim>}} above
        the element.
  \item[\texttt{vfill=\{<n>\}}] Insert an elastic \latexinline{\vskip 0pt plus <n>fill}
        before the element.  This creates a stretchable gap — a single
        \texttt{vfill=1} between two groups of elements pushes one group to
        the top and the other to the bottom.  Two elements each with
        \texttt{vfill=1} distribute remaining space evenly.
...
\begin{enumerate}
  ...
  \item Use \texttt{vspace=\{<dim>\}} for a fixed gap and
        \texttt{vfill=\{<n>\}} (integer) for a stretchable gap.
  \item A single \texttt{vfill=1} between two groups of elements pushes one
        group to the top and the other to the bottom.  Use equal integer
        weights (e.g.\ two elements with \texttt{vfill=1}) for equal
        distribution.
...
\ntaddtocover[vfill=1, halign=l]{1-1,2-1}{%
...
is executed as a \latexinline{tikzpicture[remember picture, overlay]}
covering the whole page, drawn after the text-layer minipage has been placed.
...
\begin{latexcode}
\NTAddToHook{cover/1-1/pre}{\color{white}}
\NTAddToHook{cover/text/pre}{\sffamily}
...
\begin{bashcode}
# Rebuild automatically on every save, for the school being tuned
make watch NT="school=nova/fct"
\end{bashcode}

The \verb!NT! override selects the school without editing any configuration
file, and \verb!make watch! rebuilds on every save, giving fast feedback
when tuning positions and margins.
...
\begin{enumerate}
  ...
  \item Cover margins are read; a \latexinline{tikzpicture} overlay anchors a
        \latexinline{minipage} at the page's top-left, offset by those margins.
...
\ntaddtocover[vfill=1, halign=l]{1-1}{%
  \fontsize{11}{11}\selectfont
...
% Advisers
\ntaddtocover[vfill=2, halign=l]{2-1}{%
...
\ntaddtocover[vfill=1, halign=l, status={final}]{2-1}{%
...
% Date at the bottom
\ntaddtocover[vfill=1, halign=l]{2-1}{%
  \fontsize{11}{11}\selectfont
  \GetDegreeName(\@LANG@COVER)\\
  \PrintDateISO{\GetDocDate(submission,year)-%
                \GetDocDate(submission,month)}%
...
\medskip
\noindent\textbf{4.  Test the new school} (no configuration file is edited):
\begin{bashcode}
make NT="school=myuniv/myfac"
\end{bashcode}

\medskip
\noindent\textbf{5.  Test all document types:}
\begin{bashcode}
make NT="school=myuniv/myfac,doctype=phd,docstatus=final"
make NT="school=myuniv/myfac,doctype=msc,docstatus=final"
\end{bashcode}
...
\ntaddtocover[vfill=1,      height=1.6cm, valign=t]{1-1}{degree text}
\ntaddtocover[vfill=1, height=1.25cm, valign=t,
              status={working,provisional}]{1-1}{draft message}
\ntaddtocover[vfill=1, height=1.25cm, valign=t,
              status={final}]{1-1}{approved message}
\ntaddtocover[vfill=1, height=8.5mm, valign=b]{1-1,2-1}{date}
\end{latexcode}
...
\begin{ShadedTabular}{ll}
  ...
  \latexinline{make NT="school=<univ/school>"}     & Build for a given school \\
  \bottomrule
\end{ShadedTabular}
\end{center}
