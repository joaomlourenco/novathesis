...
(\hologo{LuaTeX} allows 65536).  \novathesis{} alone claims 13 of those 16
...
Beyond the 13 slots the class uses, each of the following costs one more:
the symbols glossary, \latexinline{\listofalgorithms} and
\latexinline{\listoflistings} (the last two only when the list is actually
printed), plus whatever your own packages need (\texttt{minted}, for
instance).  The \novathesis{} demo document uses one slot too many, so it
needs \texttt{morewrites}; most real theses do not.
