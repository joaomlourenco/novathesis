...
\begin{itemize}
  ...
  \item \textbf{The matrix builds with \texttt{FASTWRITES=1} by default}, so
        \hologo{pdfLaTeX} variants skip \texttt{morewrites} and run in about
        half the time.  That means the matrix no longer exercises the
        \texttt{morewrites} path, so run \verb!make matrix FASTWRITES=0!
        from time to time to keep it covered.
