...
make matrix ONLY=nova        # only NOVA schools
make matrix ONLY=nova/fct    # one school (and its sub-programs)
make matrix ONLY=uminho,ipl  # comma-separated prefixes
\end{bashcode}

\verb!make matrix! builds every school/doctype/language/engine combination
listed in \verb!.Build/schools.conf!.  The \verb!ONLY! variable restricts
...
into a folder named with the invocation timestamp,
\texttt{YYYY-MM-DD@hh-mm-ss}, together with the \verb!.log! files of any
failed variants.
...
\begin{itemize}
  \item \textbf{Groups build sequentially.}  The variants of a group are
        very similar, so they share one auxiliary directory and are built
        one after the other: each build reuses the previous variant's
        auxiliary files (\texttt{toc}, \texttt{bbl}, \ldots) and needs fewer
        \hologo{LaTeX} passes.
  \item \textbf{The \texttt{order:} tuple sets the loop nesting} inside the
        group (the last dimension varies fastest).  It is tuned empirically
        to maximize the reuse of auxiliary files.
  \item \textbf{Each \texttt{processor:} gets its own chain.}  A group with
        \texttt{processor: pdf,lua} yields two independent sequential
        chains, with separate auxiliary directories (mixing \hologo{pdfLaTeX}
        and \hologo{LuaLaTeX} auxiliary files would spoil the reuse).
