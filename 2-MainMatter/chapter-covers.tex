...
committee) should work exclusively with \verb!0-Config/3_cover.tex!.

The cover engine is implemented in two files:
\begin{itemize}
  \item \verb!NOVAthesisFiles/StyFiles/nt-covers.sty! — the legacy
        implementation, retained for compatibility.
  \item \verb!NOVAthesisFiles/StyFiles/nt-covers-expl3.sty! — the modern
        \LaTeX3/expl3 reimplementation (the one used by default).
\end{itemize}
Both expose an identical public API; the remainder of this chapter describes
that API.
...
\begin{enumerate}
  \item \textbf{Background colour.}  A solid \verb!\pagecolor{}! fills the
        entire page.  Set with \verb!\thesiscover(id,bgcolor):={colorname}!.
  \item \textbf{Background image.}  A full-bleed graphic is stretched to
        \verb!\paperwidth! $\times$ \verb!\paperheight!.  Set with
        \verb!\thesiscover(id,image):={filename}!.
  \item \textbf{Content layer.}  A \verb!tikzpicture! overlay anchors a
        large \verb!minipage! at the top-left of the text area (offset by the
        cover margins).  Inside this minipage, elements registered with
        \verb!\ntaddtocover! are typeset top-to-bottom.  TikZ elements
        registered with \verb!tikz=true! or \verb!\ntAddToCoverTikz! are
        collected and rendered in a \emph{second} overlay \verb!tikzpicture!
...
Section~\ref{sec:covers_margins}).  All \verb!\ntaddtocover! elements are
...
comma-separated list of IDs (e.g.\ \verb!1-1,2-1!) can be passed to any
...
\verb!\margin(cover,...):={...}! family:

\begin{verbatim}
\margin(cover,top):={8.6cm}    % default top margin for all cover pages
\margin(cover,bottom):={1.5cm}
\margin(cover,left):={3.55cm}
\margin(cover,right):={2.0cm}
\margin(cover,2-1,top):={6.6cm} % override top margin for cover 2-1 only
\end{verbatim}
...
\thesiscover(1-1,bgcolor):={fctdarkblue}

% Per-doctype variant (overrides the generic key when doctype=phd)
\thesiscover(phd,1-1,bgcolor):={fctdarkgreen}

% Set a background image (file must be findable by LaTeX/graphicx)
...
Supported keys:
\begin{itemize}
  \item \verb!bgcolor! — name of a colour defined with \verb!\definecolor!.
  \item \verb!image! — file name (no extension; passed to
        \verb!\includegraphics!).  The image is automatically scaled to
        \verb!\paperwidth! $\times$ \verb!\paperheight!.
  \item \verb!textcolor! — sets \verb!\color{...}! for the entire content
...
\begin{description}
  ...
  \item[\texttt{\{cover-id-list\}}] Comma-separated list of cover IDs (e.g.\
        \verb!1-1,2-1!) to which this element is appended.
  \item[\texttt{\{code\}}] The content to typeset.  Evaluated inside a
        \verb!minipage! (unless \verb!tikz=true!; see
...
\begin{description}
  ...
  \item[\texttt{height/phd=\{<dim>\}}] Same as \texttt{height}, but only
        applied when the document type is PhD.  Overrides \texttt{height} for
        PhD documents.
  \item[\texttt{height/msc=\{<dim>\}}] Same, but for MSc documents.
...
\begin{description}
  \item[\texttt{halign=c|l|r|j}] Horizontal alignment of content inside the
        box (default: \texttt{c}).  Values: \texttt{c} = \verb!\centering!,
        \texttt{l} = \verb!\raggedright!, \texttt{r} = \verb!\raggedleft!,
...
\begin{description}
  \item[\texttt{color=\{<colorname>\}}] Foreground text colour inside the box.
        Overrides \texttt{textcolor} from \verb!\thesiscover!.
  \item[\texttt{bgcolor=\{<colorname>\}}] Background colour of the box
        (applied via \verb!\colorbox!).  Default: transparent.
\end{description}
...
\begin{description}
  \item[\texttt{vspace=\{<dim>\}}] Insert a fixed \verb!\vspace*{<dim>}! above
        the element.
  \item[\texttt{vspace=\{<n>\}}] When the value is a plain integer, insert an
        elastic \verb!\vskip 0pt plus <n>fill! instead.  This creates a
...
\begin{description}
  \item[\texttt{newpar=true|false}] Emit a \verb!\par! after the element
...
Use the \texttt{degree} filter or the internal \verb!\ifphddoc!/\verb!\ifmscdoc!
helpers when the cover layout differs between PhD and MSc documents:
...
\ntaddtocover[degree=phd, height=3cm, valign=c]{1-1}{%
  \fontsize{20}{21}\selectfont \textbf{\thedoctitle(\@LANG@COVER,main)}%
}
\ntaddtocover[degree=msc, height=3cm, valign=c]{1-1}{%
  \fontsize{17}{17}\selectfont \textbf{\thedocauthor(name)}%
}
\end{verbatim}
...
is executed inside a \verb!tikzpicture[remember picture, overlay]! that covers
the whole page, drawn after the text-layer minipage has been placed.

This means the coordinate origin is the \verb!current page! node, not the
text area, so you can position elements anywhere on the page with absolute
coordinates.  The \verb!calc! TikZ library (\verb!\usetikzlibrary{calc}!) is
...
\ntaddtocover[tikz]{1-1}{%
  \node[anchor=center]
    ...
    {\includegraphics[width=21mm]{\theuniversity(logo,neg)}};%
...
into a single \verb!tikzpicture!, in registration order.
...
\begin{description}
  \item[\texttt{<cmd>}] Optional: a TikZ command other than \verb!\node!
        (use angle brackets, e.g.\ \verb!<\draw>!).  When omitted, the
        command defaults to \verb!\node!.
  ...
  \item[\texttt{(position)}] Required: position of the node or command, in
        the TikZ coordinate system relative to \verb!current page! (parentheses).
  \item[\texttt{[rest]}] Optional: additional TikZ node syntax placed between
        the position and the final \verb!{code}! argument (square brackets,
        default empty).
  \item[\texttt{\{cover-id-list\}}] Comma-separated list of cover IDs.
  \item[\texttt{\{code\}}] Node content (when using the default \verb!\node!
...
When a command \verb!<\draw>! is specified:
\begin{verbatim}
...
\verb!\ntAddToCoverTikz! uses \verb!yscale=-1!, so the $y$-axis points
\emph{downward}.  Coordinates like \verb!(current page.north west) + (x,-y)!
...
\begin{itemize}
  ...
  \item Load cover-specific fonts (e.g.\ via \verb!\setmainfont! for
...
Enable debug mode in \verb!0-Config/1_novathesis.tex!:
...
\verb!\debuggrid! over every cover page and draws thin frames around each
text box placed by \verb!\ntaddtocover!.

You can also call \verb!\debuggrid! manually inside a cover element:
\begin{verbatim}
...
\begin{enumerate}
  ...
  \item A PDF bookmark is inserted.
  ...
  \item Background colour lookup: the engine searches the
        ...
        and calls \verb!\pagecolor!.
  \item Background image lookup: if an \texttt{image} key is found, a
        full-page \verb!tikzpicture! overlay renders it.
  \item \texttt{cover/1-1/text/pre} hook.
  \item Cover margins are read; a \verb!tikzpicture! overlay anchors a
        \verb!minipage! at \verb!current page.north west! offset by those
        margins.
  \item Inside the minipage, each element registered with
        \verb!\ntaddtocover[!…\texttt{]}\verb!{1-1}! is processed in order.
  ...
  \item \texttt{tikz=true} elements accumulated during step~8 are emitted in
        a second overlay \verb!tikzpicture!.
...
\verb!myuniv-myfac-defaults.ldf!:

\begin{verbatim}
%!TEX root=../../../template.tex
%% myuniv/myfac/myuniv-myfac-defaults.ldf

\typeout{NT FILE schools/myuniv/myfac/myuniv-myfac-defaults.ldf}%
...
\university(en) := {My University}
\university(pt) := {Minha Universidade}
\school(en)     := {My Faculty}
\school(pt)     := {Minha Faculdade}

\school(logo,en) := {myuniv-myfac-logo-en}
\school(logo,pt) := {myuniv-myfac-logo-pt}
...
\margin(cover,top):={4.0cm}
\margin(cover,bottom):={2.0cm}
...
\thesiscover(1-1,bgcolor):={myfacblue}
\thesiscover(1-1,textcolor):={white}
\thesiscover(2-1,bgcolor):={white}
\thesiscover(N-2,bgcolor):={myfacblue}
...
\ntaddtocover[tikz]{1-1}{%
  \node[anchor=north east]
    ...
    {\includegraphics[height=2cm]{\theschool(logo)}};%
}
...
\ntaddtocover[halign=l]{2-1}{%
  \includegraphics[height=1.5cm]{\theschool(logo)}\\[4mm]%
...
\verb!NOVAthesisFiles/Schools/myuniv/myfac/Images/!.
...
%%-----------------------------------------------------------------------
\subsection{NOVA/FCT: Mixed LaTeX and TikZ}
\label{sec:covers_nova_fct}
%%-----------------------------------------------------------------------

The NOVA~FCT cover (\verb!NOVAthesisFiles/Schools/nova/fct/nova-fct-defaults.ldf!)
...
\begin{itemize}
  ...
  \item PhD and MSc documents swap the order of title and author name using
        \verb!\ifphddoc! conditionals inside shared elements.
\end{itemize}

Key cover-margin setting:
\begin{verbatim}
\margin(cover,top):={8.6cm}   % leaves room for the TikZ logo band
\margin(cover,2-1,top):={6.6cm} % front page has a smaller logo band
\end{verbatim}
...
\thesiscover(\@DOCTYPE,1-1,image):=
    {nova-fct-cover-\@novafctcovercolor-front}
\end{verbatim}


%%-----------------------------------------------------------------------
\subsection{ULISBOA/IST: Pure LaTeX Box Stacking}
\label{sec:covers_ist}
%%-----------------------------------------------------------------------

The IST cover
(\verb!NOVAthesisFiles/Schools/ulisboa/ist/ulisboa-ist-defaults.ldf!)
...
\begin{tabular}{lp{8cm}}
  ...
  \hline
  \verb!\margin(cover,top):={...}!          & Cover text-area top margin \\
  \verb!\margin(cover,<id>,top):={...}!     & Per-cover override \\
  \verb!\thesiscover(<id>,bgcolor):={...}!  & Page background colour \\
  \verb!\thesiscover(<id>,image):={...}!    & Full-page background image \\
  \verb!\thesiscover(<id>,textcolor):={...}!& Default text colour \\
  \verb!\ntaddtocover[attrs]{ids}{code}!    & Register a text box element \\
  \verb!\ntaddtocover[tikz]{ids}{code}!     & Register TikZ overlay code \\
  \verb!\ntAddToCoverTikz<cmd>[opts](!…\verb!){ids}{code}! & Register a TikZ node/command \\
  \verb!\NTAddToHook{cover/<id>/pre}{...}!  & Inject code before a cover \\
  \verb!\debuggrid[color]!                  & Render a measurement grid \\
  \verb!\ntsetup{debug={cover}}!            & Enable cover debug mode \\
  \verb!python .Build/build.py ... --mode 2! & Build cover pages only \\
  \hline
\end{tabular}
\end{center}
