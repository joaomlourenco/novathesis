...
To make citations, the reference key in the BibTeX file should be used. If it is a single reference~\cite{Artho04}, use a ``\latexinline!~!'' to link the \latexinline!\cite{...}! to the word that precedes it (\ldots\latexinline!reference~\cite{Artho04}!). If you want to make multiple citations~\cite{Shavit95,Silberschatz06,Moss85}, they should be grouped within a single \latexinline!\cite{...}! and separeted by commas.

Note that the bibliography file can have as many entries as you want. Only those whose keys are referenced in the text will be included in the bibliography list.
...
\qquad\qquad
  ...
  \caption{Example of \emph{subbottom} usage}
...
\label{eq:1}
\end{equation}
which can later be referenced as equation~\ref{eq:1}
...
\begin{lstlisting}[caption=cap,label=lst:lab1,float=htbp]
if(a==b)
  puts("YESS!")
\end{lstlisting}
\fi
