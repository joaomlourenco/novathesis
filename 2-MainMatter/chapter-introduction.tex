...
\begin{enumerate}
  \item \textbf{Configuration:} Start by editing the files in \texttt{0-Config/}. Select your school, set your document type (PhD, MSc, etc.), and fill in your metadata (title, author, supervisors) in \texttt{3\_cover.tex}.
  \item \textbf{Front Matter:} Write your acknowledgments, dedication, and abstracts in the \texttt{1-FrontMatter/} folder.
  \item \textbf{Main Content:} Create separate \texttt{.tex} files for each chapter in \texttt{2-MainMatter/}. Register them in \texttt{0-Config/4\_files.tex} to include them in the build.
  \item \textbf{Bibliography:} Keep your references in \texttt{4-Bibliography/bibliography.bib}. The template uses \texttt{biblatex} to handle formatting automatically.
  \item \textbf{Build \& Iterate:} Use \texttt{make} or your editor's build command frequently to check the results.
\end{enumerate}
