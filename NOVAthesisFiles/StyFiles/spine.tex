...
\spine(date)=?{\GetDocDate(submission,year)}
...
% \typeout{spine(box,logo,len)   = \thespine(box,logo,len)}
% \typeout{spine(box,logo,align) = \thespine(box,logo,align)}
% \typeout{spine(box,author,len) = \thespine(box,author,len)}
% \typeout{spine(box,title,len)  = \thespine(box,title,len)}
% \typeout{spine(box,date,len)   = \thespine(box,date,len)}
...
\newcommand{\@checkspinelen}[1][\paperheight]{%
  ...
  \DEBUG[\@tempdima]{SPINELEN MARGIN [left]}%
  \DEBUG[\@tempdimb]{SPINELEN MARGIN [right]}%
  \@tempdima=\dimexpr\@tempdima+\@tempdimb\relax%
  \DEBUG[\@tempdima]{SPINELEN ACCUMULATED}%
  \StrCount{\thespine(order)}{,}[\@nBoxSep]%
  \@for\myi:=\expanded{\thespine(order)}\do{%
    \@tempdimb=\thespine(box,\myi,len)\relax%
    \DEBUG[\@tempdimb]{SPINELEN BOXLEN [\myi]}%
    \@tempdima=\dimexpr\@tempdima+\@tempdimb\relax%
    \DEBUG[\@tempdima]{SPINELEN ACCUMULATED}%
  }
  \DEBUG[#1]{SPINELEN MAX}%
  \@spinelen=\@tempdima%
}
...
% \displayspinealyout
\@checkspinelen
% \DEBUG[\paperheight]{PAPER HEIG}
% \DEBUG[\@spinelen]{SPINELEN[FINA}
% \PAUSETHIS
...
\@for\myi:=\expanded{\thespine(order)}\do{
  ...
  \DEBUG[\thespine(box,\myi,left)]{BOX[\myi,left]}
  \DEBUG[\thespine(box,\myi,len)]{BOX[\myi,len]}
  \DEBUG[\thespine(box,\myi,right)]{BOX[\myi,right]}
}%
