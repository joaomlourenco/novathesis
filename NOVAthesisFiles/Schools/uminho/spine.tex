...
\newcommand{\spinetext}{%
  ...
  \hspace*{10mm}%
  \GetSpine(font)%
  \parbox{197mm}{UMinho~$|$~\GetSpine(year)\hfill\GetSpine(author)\hfill\textbf{\GetSpine(title)}}%
  \parbox{90mm}{~}%
}
...
\newcommand{\@ntprintspine}{%
  ...
  % draw image
    ...
    %                          {\GetThesisCover(spine,image)}%
    %         };
    \node (univ) [inner sep=0,anchor=north,yshift=0.5pt] at (current page.north)
            {
                \rotatebox{-90}{\includegraphics[width=\novathesis@spinewidth,angle=90]{\GetSpine(logo,university)}\hspace{-0.5pt}%
                \includegraphics[width=\novathesis@spinewidth,angle=90]{\GetSpine(logo,school)}}
             };
    % \node (schl) [inner sep=0,below of=univ]%
    %         {\includegraphics[width=\novathesis@spinewidth]%
    %                          {\GetSpine(logo,school)}};
